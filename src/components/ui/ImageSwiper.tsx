    ...
    >
      {cardOrder.map((originalIndex, displayIndex) => (
        <article
          ...
          className="image-card absolute cursor-grab place-self-center overflow-hidden rounded-2xl border border-line bg-mist shadow-xl shadow-ink/10 will-change-transform active:cursor-grabbing"
          style={
            {
              ...
              height: cardHeight,
              transform:
                "perspective(var(--card-perspective)) translateZ(calc(-1 * var(--card-z-offset) * var(--i))) translateY(calc(var(--card-y-offset) * var(--i))) translateX(var(--swipe-x, 0px)) rotateY(var(--swipe-rotate, 0deg))",
            } as React.CSSProperties
          }
        >
